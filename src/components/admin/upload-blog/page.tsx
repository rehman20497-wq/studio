...
import { UploadCloud, FileText, Type, Image as ImageIcon, CheckCircle, User, MessageSquare, BookOpen, Shield, Globe, Tag, Search } from 'lucide-react';
...
const blogPostSchema = z.object({
  ...
  keywords: z.string().optional(), // We'll convert string to array on submit
...
function UploadBlogContent() {
  ...
  const onSubmit = async (data: BlogPostFormValues) => {
    if (!firestore) {
        toast({ variant: 'destructive', title: 'Database connection failed', description: 'Could not connect to Firestore.' });
        return;
    }
    try {
        const blogCollection = collection(firestore, 'blog_posts');
        const keywordsArray = data.keywords ? data.keywords.split(',').map(k => k.trim()) : [];
        const tagsArray = data.tags ? data.tags.split(',').map(t => k.trim()) : [];
        ...
        <div className="p-4 sm:p-8 md:p-12">
            ...
            <div className="mt-12">
                <form onSubmit={handleSubmit(onSubmit)} className="max-w-5xl mx-auto space-y-16" onMouseMove={handleMouseMove}>
                    ...
                    <SectionWrapper title="Search Engine Optimization (SEO)" step={2} icon={Search}>
                        <div className="space-y-4">
                            <div>
                                <label htmlFor="metaTitle" className="font-semibold text-zinc-700">SEO Meta Title</label>
                                <Input id="metaTitle" placeholder="Keyword rich title for Google" {...register('metaTitle')} />
                            </div>
                            <div>
                                <label htmlFor="metaDescription" className="font-semibold text-zinc-700">SEO Meta Description</label>
                                <Textarea id="metaDescription" placeholder="Brief summary for search results (150-160 chars)" {...register('metaDescription')} />
                            </div>
                            <div>
                                <label htmlFor="keywords" className="font-semibold text-zinc-700 flex items-center gap-2">
                                    <Search className="w-4 h-4" /> Focus Keywords
                                </label>
                                <Input id="keywords" placeholder="cloud, AI, outsourcing (comma separated)" {...register('keywords')} />
                            </div>
                            <div>
                                <label htmlFor="tags" className="font-semibold text-zinc-700 flex items-center gap-2">
                                    <Tag className="w-4 h-4" /> Tags
                                </label>
                                <Input id="tags" placeholder="tech, news, guide (comma separated)" {...register('tags')} />
                            </div>
                        </div>
                    </SectionWrapper>

                    <SectionWrapper title="Featured Image" step={3} icon={ImageIcon}>
                        <ImageUploader
                        id="featured-image"
                    ...
                    <SectionWrapper title="Content" step={4} icon={FileText}>
                        <Controller
                            name="content"
                            control={control}
                            render={({ field }) => (
                    ...
                    <SectionWrapper title="Pull-Quote (Optional)" step={5} icon={MessageSquare}>
                        ...
                        <Controller
                            ...
                            render={({ field }) => (
                                ...
                                <Label htmlFor="published-status" className="flex flex-col">
                                    ...
                                    {field.value ? 'This post will be visible to the public.' : 'This post will be saved as a draft.'}
                                    </span>
                                </Label>
                                </div>
                            )}
