...
const cardData = [
  {
    icon: '/drt.svg', // You can replace with a suitable network icon
  ...
  {
    icon: '/eye.svg', // Replace with a monitoring-related icon
  ...
  {
    icon: '/sea.svg', // Replace with a security-related icon
...
const ServiceCard = ({ card }: { card: (typeof cardData)[0] }) => (
  <motion.div className="relative" variants={cardItemVariants}>
    ...
    <motion.div
      className="hidden md:block absolute bg-[#e64628] rounded-2xl overflow-hidden"
      style={{
        width: '85%',
    height: '95%',
    top: '6%',
      ...
      <svg width="100%" height="100%" xmlns="http://www.w3.org/2000/svg" className="opacity-40">
        <defs>
          <pattern id="circle-pattern" patternUnits="userSpaceOnUse" width="100" height="100">
            <circle cx="50" cy="50" r="47" stroke="#fd9c8a" strokeWidth="12" fill="none" />
          </pattern>
        </defs>
...
>
      ...
      <div className="flex items-center mb-2">
        <div className="w-12 h-12 bg-[#fd9c8a] rounded-full flex-shrink-0" />
        <Image
          src={card.icon}
          alt={card.title}
          width={50}
