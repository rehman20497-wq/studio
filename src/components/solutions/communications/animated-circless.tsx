...
/*
  T SHAPE:
  Row 0 → 7 circles
  Row 1–5 → 1 circle each
  Vertical line aligned under 4th circle (index 3)
*/
const ROW_PATTERN = [7, 1, 1, 1, 1, 1];

const SMALL_BOX_SIZE = SMALL_CIRCLE_RADIUS * 2 + SPACING;
const TOTAL_WIDTH = 7 * SMALL_BOX_SIZE;
const TOTAL_HEIGHT = ROW_PATTERN.length * SMALL_BOX_SIZE;
const PADDING = 40; // increase this number to add more space around the SVG
...
const generateRightAlignedCircles = () => {
  ...
  // Center index of 7 circles → 4th circle → index 3
  const CENTER_INDEX = 3;

  ROW_PATTERN.forEach((cols, rowIndex) => {
    let startX;

    // Top horizontal bar (7 circles)
    if (rowIndex === 0) {
      const rowWidth = cols * SMALL_BOX_SIZE;
      startX = (TOTAL_WIDTH - rowWidth) / 2; // centered
    } 
    // Vertical stem (1 circle rows)
    else {
      // Align under 4th circle of top row
      startX = (TOTAL_WIDTH / 2) - (SMALL_BOX_SIZE / 2);
    }
...
/* ------------------ MAIN ------------------ */
export default function AnimatedCircless() {
  ...
  return (
    <div className="w-full h-full flex items-center justify-center">
      <svg
  ref={scope}
  viewBox={`${-PADDING} ${-PADDING} ${TOTAL_WIDTH + 2 * PADDING} ${TOTAL_HEIGHT + 2 * PADDING}`}
